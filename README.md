...
| `/panic` | Ставит pipeline текущей доски на паузу |
...
- **Cheap heartbeat first** — без wake conditions агент делает только дешёвый check-in и пропускает pre-flight/context pull
...
**Прогресс: ~97%** — ядро, guarded pipeline, operator control и heartbeat economy уже работают; phase-9 polish всё ещё в процессе.
