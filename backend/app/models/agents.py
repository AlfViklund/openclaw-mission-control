...
class Agent(QueryModel, table=True):
    ...
    agent_auth_mode: str = Field(default="legacy_hash")
    agent_token_version: int = Field(default=1, ge=1)
    pending_agent_token_version: int | None = Field(default=None)
    agent_auth_last_synced_at: datetime | None = Field(default=None)
    agent_auth_last_error: str | None = Field(default=None, sa_column=Column(Text))
